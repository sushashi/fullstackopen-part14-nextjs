...
import { addUser, updateToken, addReadingList, updateRead, getBlogByReadingListId} from "../services/users";
...
export const markAsRead = async (FormData: FormData) => {
  ...
  const blog = await getBlogByReadingListId(readingListId);
  revalidatePath("/me");
  revalidatePath(`/blogs/${Number(blog?.blogId)}`);
};